...
public class ChoicenessAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        ChoicenessEntity.T1467284926140Bean choicenessEntity = (ChoicenessEntity.T1467284926140Bean) getItem(position);
        if (choicenessEntity != null) {
            Picasso.with(context).load(choicenessEntity.getImgsrc()).into(viewHolder.newsImg);
            viewHolder.newsTitle.setText(choicenessEntity.getTitle());
            viewHolder.newsDate.setText(choicenessEntity.getSource());
            viewHolder.newsReply.setText(choicenessEntity.getReplyCount() + "" + "跟帖");
        }
        return convertView;
    }
